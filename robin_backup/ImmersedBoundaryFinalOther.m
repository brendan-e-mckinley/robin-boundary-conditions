...
for j = 1:size
    ...
    tol = .01*dx*dx;
    D = @(u) applyLap(u,D2,N(j));
    S = @(q) spreadQ(X,Y,xib,yib,q,delta);
    J = @(u) assembleJ(X,Y,theta_int,u,delta,dx);
...
%    ibmag = 1/sqrt(1 + uib(k)^2);
    %quiver3(xib(k), yib(k), uib(k), ibmag*cos(theta(k)), ibmag*sin(theta(k)), uib(k) + uib(k)/sqrt(1 + uib(k)^2), 'Color', 'r');
%    quiver3(pi, pi, 0, pi - xib(k), pi - yib(k), uib(k) + uib(k)/sqrt(1 + uib(k)^2), 'Color', 'r');
%    end
%    hold on
%end
