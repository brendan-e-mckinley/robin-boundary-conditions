...
%endpoints = normal - cos(2.*xib).*cos(2.*yib);
normal = (interpPhi(X,Y,xouter,youter,u_approx,delta) - interpPhi(X,Y,xinner,yinner,u_approx,delta))/(2*dib);
for k = 1:length(uib)
    if (mod(k, 2) == 0)
